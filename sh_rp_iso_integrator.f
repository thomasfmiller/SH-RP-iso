...
c      the program uses only even number of beads
       if ((nbead.ne.1).and.(mod(nbead,2).ne.0)) then
        stop'Odd number of beads!'
       end if
...
c     given centroid initial velocity and position returns a ring-polymer with the rest of the modes thermalized 
      subroutine Samplefree(xcin,vcin)
      implicit none
...
c     draw a random number and deside whether to hop
      ...
      if(rrr.lt.prob) call SurfaceSwitch()
...
c     Gaussian random number deviates
      double precision function gdev(sigma)
      double precision sigma,rrr,rsq,r1,r2
1      call random_number(rrr)       
       ...
       gdev = r1*sqrt(-2.d0*log(rsq)/rsq)*sigma
      return
      end function gdev
