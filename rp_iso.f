...
c       The evaluation uses Bell's algorithm - see the original paper for a reference
        holel(:,:,1)=m(:,:,1)
        do i = 2, nbead-1
...
c      unitary transformation matrix from diabatic to adiabatic basis
       cs=dcos(th)
       sn=dsin(th)
       dcs=-sn*dth
       dsn=cs*dth
